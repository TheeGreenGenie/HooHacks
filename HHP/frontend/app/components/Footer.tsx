...
export default function Footer() {
  return (
    ...
    >
      <div className="mx-auto max-w-7xl overflow-hidden py-10 px-6">
          ...
          {footerNavigation.main.map((item) => (
            <div key={item.name} className="px-5 py-2">
              <Link
                ...
                style={{ color: "#6B4820" }}
                onMouseEnter={e => (e.currentTarget.style.color = "#C8A060")}
        ...
        <div className="mt-6 flex justify-center space-x-5">
          {footerNavigation.social.map((item) => (
            <a key={item.name} href={item.href} style={{ color: "#4A2C10" }}
              onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = "#8B6030")}
              onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = "#4A2C10")}
            >
              <span className="sr-only">{item.name}</span>
              {item.icon()}
            </a>
